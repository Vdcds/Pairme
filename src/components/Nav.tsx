      ...
      <div className="mx-auto flex h-[68px] max-w-7xl items-center gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {!isLoaded ? (
            ...
            <div className="grid h-9 w-9 place-items-center rounded-xl border border-border bg-card/70 p-0.5">
              <UserButton afterSignOutUrl="/" appearance={{ elements: { avatarBox: "h-7 w-7" } }} />
            </div>
          ) : (
            <div className="hidden items-center gap-1.5 sm:flex">
              <SignInButton mode="modal"><Button size="sm" variant="ghost" className="h-9 rounded-xl border border-border bg-card/65 px-3.5 text-muted-foreground hover:bg-secondary hover:text-foreground">Sign in</Button></SignInButton>
              <SignUpButton mode="modal"><Button size="sm" className="rose-gradient h-9 rounded-xl border-0 px-3.5 font-semibold text-primary-foreground">Join</Button></SignUpButton>
            </div>
          )}
